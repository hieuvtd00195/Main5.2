...
{
    if (g_iChatInputType == 1)
        g_pSingleTextInputBox->GetText(InputText[0]);  // Đảm bảo `GetText` trả về kiểu `wchar_t`
    ...
    {
        SendRequestCreateCharacter(InputText[0], CharacterView.Class, CharacterView.Skin);  // Giả sử `SendRequestCreateCharacter` hỗ trợ `wchar_t`
        rUIMng.HideWin(this);
